...
from dj_jenkins.core.j_pipeline_build import JPipelineBuild
...
class JPipeline:
    def __init__(self,
                 ...
                 init: bool = False):
        self._server = server
        self._class = JenkinsType.PIPELINE.value
    ...
    def builds(self, next_build_number: int = -1) -> List[JPipelineBuild]:
        objs = []
        for bld in self.build_numbers:
            if (next_build_number != -1 and bld >= next_build_number) or next_build_number == -1:
                obj = JPipelineBuild(self._server, self.fullname, bld, init=True)
                objs.append(obj)
        return objs

    def to_model(self) -> PipelineModel:
        if not PipelineModel.objects.filter(
                is_deleted=False,
            ...
            obj = PipelineModel.objects.filter(
                is_deleted=False,
                fullname=self.fullname).first()
